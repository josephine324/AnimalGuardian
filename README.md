...
The project includes several PowerShell scripts to help with development, especially when dealing with OneDrive file locking issues during Flutter builds.

### Available Scripts

#### 1. OneDrive Management Scripts

##### `frontend/pause_onedrive.ps1`
Pauses OneDrive sync temporarily to prevent file locking during builds.
...
- Prevents file locking during Flutter builds
- Shows status messages

##### `frontend/resume_onedrive.ps1`
Resumes OneDrive sync after builds are complete.
...
**What it does:**
- Restarts the OneDrive process
- Resumes normal file syncing

##### `frontend/run_without_onedrive.ps1`
Automated script that pauses OneDrive, runs the Flutter app, and resumes OneDrive automatically.
...
# Or specify a device:
.\run_without_onedrive.ps1 chrome
```

**What it does:**
- Pauses OneDrive
- Optionally cleans build directory
- Runs Flutter app on specified device
- Automatically resumes OneDrive when done

##### `frontend/exclude_build_from_onedrive.ps1`
Attempts to exclude the build folder from OneDrive sync (permanent solution).
...
**What it does:**
- Applies file attributes to exclude build folder
- Creates `.onedriveignore` file (if supported)
- Provides manual instructions for OneDrive settings

**Note:** This script provides instructions for manual exclusion, as OneDrive doesn't fully support programmatic exclusion.

#### 2. Build Management Scripts

##### `frontend/force_clean_build.ps1`
Forcefully cleans the build directory, handling locked files.
...
**What it does:**
- Stops Java, Gradle, Dart, and Flutter processes
- Removes build directory with multiple retry methods
- Cleans Flutter cache
- Handles file locking issues

**When to use:**
- When you get "Unable to delete directory" errors
- When build files are locked by processes
- Before a fresh build

##### `frontend/build_workaround.ps1`
Comprehensive workaround script that handles multiple build issues.

**Usage:**
```powershell
cd frontend
.\build_workaround.ps1
# Or specify a device:
.\build_workaround.ps1 chrome
```

**What it does:**
- Pauses OneDrive
- Stops Java/Gradle processes
- Removes problematic directories with retries
- Builds with workaround options

**When to use:**
- When standard build fails
- When dealing with permission_handler_android issues
- As a last resort before moving project

#### 3. Project Management Scripts

##### `move_project_outside_onedrive.ps1`
Moves the entire project outside OneDrive to avoid all file locking issues.
...
**What it does:**
- Copies project from OneDrive to `C:\Projects\AnimalGuardian`
- Excludes build directories and cache files
- Uses robocopy for efficient copying
- Provides next steps after completion

**When to use:**
- If you frequently encounter OneDrive locking issues
- For active development work
- Best long-term solution

**Note:** Update the source and target paths in the script to match your setup.

### Quick Reference

#### Before Building:
```powershell
# Option 1: Pause OneDrive manually
cd frontend
.\pause_onedrive.ps1
...
#### After Building:
```powershell
# Resume OneDrive (if you paused it manually)
cd frontend
.\resume_onedrive.ps1
```

### OneDrive Build Issues - Solutions
...
1. **Exclude Build Folder from OneDrive (Recommended - Permanent)**
   - Run `exclude_build_from_onedrive.ps1` for instructions
   - Or manually: Right-click build folder → "Always keep on this device"
   - Or via OneDrive Settings → Sync and backup → Choose folders → Uncheck build

2. **Pause OneDrive During Builds (Temporary)**
   - Use `pause_onedrive.ps1` before building
   - Use `resume_onedrive.ps1` after building
   - Or use `run_without_onedrive.ps1` for automation

3. **Move Project Outside OneDrive (Best for Development)**
   - Run `move_project_outside_onedrive.ps1`
   - This permanently solves the issue for active development

### Script Locations

All scripts are located in:
- `frontend/` - OneDrive and build management scripts
- Root directory - Project management scripts

### Permissions

Some scripts may require Administrator privileges:
- `exclude_build_from_onedrive.ps1` - For file attribute changes
- `move_project_outside_onedrive.ps1` - For copying files

If you encounter permission errors, right-click PowerShell and select "Run as Administrator".

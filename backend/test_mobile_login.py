#!/usr/bin/env python
"""Test mobile app login authentication."""
import os
import sys
import django
...
if user:
    ...
    if auth_user:
        print(f"Authenticate SUCCESS: {auth_user.username}")
    else:
        print(f"Authenticate FAILED")
        # Try with username
        auth_user2 = authenticate(username=user.username, password=password)
        if auth_user2:
            print(f"Authenticate with username SUCCESS: {auth_user2.username}")
        else:
            print(f"Authenticate with username FAILED")
else:
    print(f"\nUser NOT found with phone: {phone_number}")
